...
class StatementImportTests(unittest.TestCase):
    def test_pdf_text_and_image_preview_are_extracted_in_memory(self):
        ...
        with self.assertRaises(statements.StatementError):
            statements.document_pages(text_pdf(), 2, 3)
    ...
    def test_extraction_uses_images_only_when_text_is_unavailable(self):
        ...
        for text, expected_image in [('', True), ('A' * 100, False)]:
            with patch.object(statements, 'local_model', return_value=response) as model:
                statements.extract_page({'number': 1, 'text': text, 'image': 'example'}, 'credit', '2026-09-01', '2026-09-30')
    ...
    def test_row_validation_rejects_guessed_or_invalid_values(self):
        ...
        self.assertEqual(statements.validate_row(row, '2026-09-01', '2026-09-30')['amount'], 1234)
        self.assertEqual(statements.validate_row({**row, 'direction': 'money_in'}, '2026-09-01', '2026-09-30')['amount'], -1234)
        for change in [{'date': '2026-08-31'}, {'date': '2026-09-31'}, {'amount': 'NaN'}, {'amount': '1e4'}, {'amount': '-12.34'}, {'amount': '12.345'}, {'amount': '0.00'}, {'direction': ''}, {'description': ''}]:
            with self.subTest(change=change), self.assertRaises(statements.StatementError):
                statements.validate_row({**row, **change}, '2026-09-01', '2026-09-30')
